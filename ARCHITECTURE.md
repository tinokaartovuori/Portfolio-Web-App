...
There are **no tests and no ESLint config** in this repo — don't go looking for them, and don't invent a test command. Prettier is the only enforced style tool (`.prettierrc.json`: no semicolons, single quotes, 2-space, trailing commas; `prettier-plugin-tailwindcss` sorts Tailwind classes).
...
- `disableScroll.ts` — currently **not registered anywhere**; dead until wired up.
...
- `store/threeObjectState.ts` — registry of DOM elements that want a 3D counterpart: `threeElementTracker` (element + object-type string) and `threeImageTracker` (image elements).
- `store/themeState.ts` — mirrors the color mode for non-Tailwind consumers.
...
- `<ThreeImage threeReference="uniqueId" imageUrl="…" />` — renders an `invisible` `<img>` (it exists only to be measured and to supply the texture URL) and registers it.
...
- `three-components/ElementManager.ts` → maps the `object` string to a class (`IntroRectangle`, `CrazyPlane`) in a chain of `if` blocks in `loadElements()`. **This is where a new object type gets registered.**

Update flow, all via `watch`:
- `watch(scrollY)` → `imageManager.updateImages(scrollYSpeed)` + `elementManager.updateElementPositions()`
- `watch(aspectRatio)` (from `useWindowSize`) → full update + camera/renderer resize
...
Every object class (`WavyImage`, `IntroRectangle`, `CrazyPlane`) repeats the same pattern — copy it when adding new ones:
...
An object class must extend `Object3D` and expose `update()`, `updatePosition()`, `updateAspectRatio()` and `dispose()` — `ElementManager` calls all four blind, and a missing method will throw at runtime.
...
- Debug `console.log` leftovers in `smooth-scrollbar-plugins/disableScroll.ts:19` and unreachable logs after the `return` in `speedControl.ts`.
- `CrazyPlane.ts` is marked work-in-progress and renders a flat red `MeshBasicMaterial`.
- README progress GIF links point at the old repo (`tinokaartovuori/my-portfolio`) and are broken here.
