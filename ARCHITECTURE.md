...
- **The glass/refraction work is not started.** The scene still has no lights, no environment map and no tone mapping — every material is unlit (the "lights" of section 10 are unlit sprites and washes, not light sources). The hooks are in place (`scene` for lights and `environment`, `Scenario.enablePostProcessing()` for screen-space passes, real 3D tilt on the images so lighting will read), but it remains a prerequisite chain, not a tuning job. Drifting refractive panes over the light cloud and the images were considered as the next step in depth and would be the first use of the post-processing hook.
...
**Every number that decides how the site feels lives in `motion.config.ts`** — scroll spring and speed cap, keyboard step, rubber-band stiffness, the shared scroll feel, the theme blend rate, the palette, the page trail shares, image trail, bubble depth, hover lift, lens size, glitch, film grain, the light fields, edge glows, the light cloud, wire shapes, index rows, marquee and magnetic pull. Tune there; the motion code reads it and nothing else should carry a magic constant.
...
- `three-components/LightCloud.ts` → the one thing in the scene not pinned to an element, owned directly by the canvas component: an `InstancedMesh` of large soft light quads spread through the whole page at different depths (section 10). It is updated after the managers in the `transform` stage, told about resizes, and disposed with the rest.
...
Every object class (`WavyImage`, `LightField`, `EdgeGlow`, `WireShape`) repeats the same pattern — copy it when adding new ones:
...
**`EdgeGlow`** is light spilling in from the page's edge at each project image, in the same palette (`accent` and `cool` alternate down the list): a plate from just outside the viewport edge to a little past the image's inner edge, set back at `-depth`, drawing a wash that fades away from the edge and is thickened and thinned by slow value noise so its shape is organic, not the image's outline. It trails its box through the image's own spring so the two move as one, and brightens under the pointer. The `variant` is `<hue>-<side>`; the side is the page edge nearest the image on wide screens and simply alternates on narrow ones. The plate's borders (other than the one off screen) are faded to nothing.
...
- **The light cloud** (`three-components/LightCloud.ts`) — the page's depth: 18 large, soft lights in the hero's palette at depths from well behind the page to a little in front of it. Each is anchored to a scroll position and moves with the page **in world space**, so the parallax is the camera's own — the deep ones cross the screen slowly, the near ones quickly, and the near ones are drawn larger and softer, like a lens out of focus. A mesh off the z=0 plane normally compensates for perspective (section 6); this one deliberately does not, since the perspective is the point. Scroll energy stretches them, the trail drags them (scaled by depth), the pointer pulls the ones near it through a slow spring, and the rubber band carries them with the page. Instanced quads rather than `Points`: `gl_PointSize` is capped low on some GPUs. Deterministic per index, so a rebuild does not reshuffle them; still under reduced motion.
