...
**Every number that decides how the site feels lives in `motion.config.ts`** — scroll spring and speed cap, keyboard step, rubber-band stiffness, the shared scroll feel, the theme blend rate, the palette, the page trail shares, image trail, bubble depth, hover lift, lens size, glitch, film grain, the light fields, edge glows, wire shapes, index rows, marquee and magnetic pull. Tune there; the motion code reads it and nothing else should carry a magic constant.
