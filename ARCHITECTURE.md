...
- **The backdrop** (`three-components/Backdrop.ts`) — the page's depth, and the first lit thing in the scene. Far behind the page (z = −650) a wide matte surface covering the whole document, **faceted**: a grid whose vertices are jittered in the plane and raised by an undulation plus per-vertex noise, left unindexed so every triangle has its own normal and drawn `flatShading`, a low relief of geometric facets. On it fall real `PointLight`s — one beside each project image on its page-edge side (`accent` and `cool` alternating, both pulled most of the way to white so the light reads as light rather than colour), one following the pointer through a slow spring — and they are dim: what shows is a quiet geometric patch of light near each image on a page that stays dark or light. Nothing glows. The images, the light field and the wire shapes are unlit materials and take none of it. **Unlit, the surface is exactly the page colour**: the material is a flat grey albedo and the ambient light is set to `π × page / albedo` per channel each frame, on either theme, so the CSS background is never seen to differ from the surface. Light intensities are candela (irradiance falls with distance²; distances here are hundreds of px, so the numbers are in the hundreds of thousands); on the light theme the lights can only brighten an already light surface, so they are set stronger there and still show less. The lights are a fixed pool: the lit shader compiles for that count and must not recompile when a page has fewer images. The surface moves with the page in world space, so the parallax is the camera's own; the lights come on as the hero scrolls away (`heroHeight` in the `FrameContext`), and a page without a hero has them on from the start. Earlier attempts, in order: an "edge glow" sprite per image and a cloud of soft quads at depths (both read as blurred rectangles), then smooth pebble-like forms under saturated lights (too colourful, too much like a 3D showcase). The answer to "make it look 3D" was to light something; the answer to "too much" was to light it less, and to light facets.
