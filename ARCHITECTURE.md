...
- **The glass/refraction work is not started.** The backdrop (section 10) is the first lit thing in the scene — an ambient light and coloured point lights on standard materials — but there is still no environment map and no tone mapping, and the images, the light field and the wire shapes are unlit. The hooks are in place (`scene.environment`, `Scenario.enablePostProcessing()` for screen-space passes, real 3D tilt on the images so lighting will read), but it remains a prerequisite chain, not a tuning job. Drifting refractive panes over the backdrop and the images were considered as the next step in depth and would be the first use of the post-processing hook.
...
**Every number that decides how the site feels lives in `motion.config.ts`** — scroll spring and speed cap, keyboard step, rubber-band stiffness, the shared scroll feel, the theme blend rate, the palette, the page trail shares, image trail, bubble depth, hover lift, lens size, glitch, film grain, the light fields, the backdrop's surface, forms and lights, wire shapes, index rows, marquee and magnetic pull. Tune there; the motion code reads it and nothing else should carry a magic constant.
...
- `three-components/Backdrop.ts` → the lit part of the scene, not pinned to any element and owned directly by the canvas component: an ambient light, a wide undulating matte surface far behind the page, a few matte forms between, and a pool of coloured `PointLight`s — one beside each project image, one under the pointer (section 10). It is updated after the managers in the `transform` stage (it reads the images' measured boxes to place their lights), told about resizes, and disposed with the rest.
...
The canvas wrapper is `fixed inset-0 z-0`; page content sits above it at `z-10`. Nothing here relies on a third-party stylesheet for stacking any more. Note that below the hero the backdrop's surface covers the whole viewport, so the wrapper's CSS colour is only ever seen through the hero and during the first frames; the surface is kept exactly that colour where unlit (section 10).
...
- **The backdrop** (`three-components/Backdrop.ts`) — the page's depth, and the first lit thing in the scene. Far behind the page (z = −650) a wide matte surface, undulating by a few sines and covering the whole document; between it and the page five matte pebble-like forms (a subdivided icosahedron pushed in and out by sines, vertices merged first or the normals come out flat); and real coloured `PointLight`s — one beside each project image on its page-edge side (`accent` and `cool` alternating), one following the pointer through a slow spring. Nothing glows: what shows is light falling on a surface, with shading and distance. The images, the light field and the wire shapes are unlit materials and take none of it. **Unlit, the surface and the forms are exactly the page colour**: the material is a flat grey albedo and the ambient light is set to `π × page / albedo` per channel each frame, on either theme, so the forms are invisible until a light passes them and the CSS background is never seen to differ from the surface. Light intensities are candela (irradiance falls with distance²; distances here are hundreds of px, so the numbers are in the hundreds of thousands). The lights are a fixed pool: the lit shader compiles for that count and must not recompile when a page has fewer images. Everything but the pointer light is anchored below the hero and moves with the page in world space, so the parallax is the camera's own; the lights come on as the hero scrolls away (`heroHeight` in the `FrameContext`), and a page without a hero has them on from the start. Two earlier attempts were sprites — an "edge glow" plate per image and a cloud of soft quads at depths — and both read as blurred rectangles; the answer to "make it look 3D" was to light something.
...
- **Index rows** (`ProjectIndex.vue`) — a box that a `WireShape` is pinned to, a rule that draws itself as the row enters the viewport, and the number in mono. On wide screens the row is reversed (`flip`) so the solid sits on the side opposite the image; on narrow ones it is always on the left. Measured from the **row's** rect, never the rule's: a rect includes the element's own transform.
