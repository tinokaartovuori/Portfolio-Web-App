...
Every object class (`WavyImage`, `LightField`, `GlowPlate`, `WireShape`) repeats the same pattern — copy it when adding new ones:
...
**`GlowPlate`** is one soft light behind each project image, in the same palette (`accent` and `cool` alternate down the list), on a plate 1.6× the image set back at `-depth`, trailing its box through a looser spring than the image so the two separate while scrolling and rejoin with a settle, and brightening under the pointer. The glow is the shape of the image: full inside the image's outline, falling off as a gaussian of the signed distance from it (in screen px, so the plate's depth stays out of the shader), so it reads as a rectangle's light rather than an ellipse behind one. The plate edge is faded to nothing: a tail with any alpha left at the edge reads as a second rectangle.

**`WireShape`** is the solid in each index row: a cube, a pyramid, an octahedron or an icosahedron (the `variant`), drawn as a `LineSegments` child built from an `EdgesGeometry` — the mesh itself has an empty geometry and an invisible material, because `wireframe: true` would also draw every face diagonal. It turns slowly at rest and faster with the scroll, in the scroll's direction, and under the pointer grows, brightens and tips toward it. It draws in the text colour (`palette.base`), so it reads as typography.
...
- **Grain** (`GrainOverlay.vue`, in the layout) — sparse grains over the whole viewport, canvas and text alike, so the page reads as one surface (grain on the hero and the images alone would read as an artefact of those). A colour matrix turns SVG noise into a flat colour whose alpha is zero for most values, so it speckles without lifting the page colour; black grains on the light page, white on the dark. Kept faint and slow: a stepped `transform` animation (not `background-position`) keeps it on the compositor, and its opacity and step rate are the whole difference between texture and distraction. Still under reduced motion. It is above the bars (`z-[60]`) and below the cursor. A post-processing pass was rejected: it would cost a full-screen target every frame and grain only the canvas.
...
- **Index rows** (`ProjectIndex.vue`) — a box on the left that a `WireShape` is pinned to (with a little parallax on the box; the mesh follows because it measures the box), a rule that draws itself as the row enters the viewport, and the number in mono at the end. Measured from the **row's** rect, never the rule's: a rect includes the element's own transform.
